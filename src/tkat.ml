...
let params = [
    ...
    ("-no-opt", Arg.Unit (fun n -> optimize := false), "Disable rule minimization");
  ...
  try begin
    ...
    if !test then run_unit_tests ()
      ...
      (if is_local then
        ...
        begin
          let dfa, nqrys = Automata.instrument !optimize tt in
          nqueries := nqrys;
          ...
          sz_qry := sz_q;
          let (rs, rs_opt1, rs_opt2, ntags) = Profile.profile "[Rule Generation]" Fdd.rules fdd in
          rules := (if !optimize then rs_opt2 else rs);
          num_rules := List.length rs;
          num_rules_opt1 := List.length rs_opt1;
          num_rules_opt2 := List.length rs_opt2;
          num_tags := ntags;
