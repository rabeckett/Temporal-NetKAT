  ...
  let determinize (auto: t) : t = 
    ...
    let trans ls  = 
      ...
      let all = NatSet.fold (fun l acc -> 
        ...
        Fdd.apply int_mem TInt.union t acc) ls base in 
    ...
    let create (trm: term) : t =
      ...
      let trans l =
        ...
        let partial (d,l,k) acc = 
          ...
          Fdd.apply int_mem TInt.union dl acc in 
    ...
    let add_accepting_state ~only_accept ~final_loop (auto: t)  = 
      ...
      let trans j = 
        if j = 0 then 
          ...
          Fdd.apply int_mem TInt.union (lookup auto.trans j) new_t in 
...
module Compile = struct 
  ...
  let add_state_tag trans = 
    let aux_inner (i,u) = 
      let s = Syntax.State (string_of_int i) in 
      Update.add s u in
    let aux us =
  ...
  let compile auto = 
    ...
    let aux auto i = 
      if is_link auto i then ()
        ...
        let t = adjust_next_hop auto i |> add_state_tag in
...
module Test = struct 
  ...
  let assert_equal x y =
    let dfax, _ = instrument x in 
    let dfay, _ = instrument y in 
